...
vec4 hitCube(vec3 ray, vec3 pos, vec3 invSize, float lod, samplerCube tex)
{
	...
	return vec4(1.0);//textureCubeLod(tex, tc, lod).rgb, 1.0);
}
...
void main()
{
  ...
  #else
	vec3 cubeLightColor = vec3(1.0);//textureCubeLod(u_CubeMap, R + parallax, ROUGHNESS_MIPS * roughness).rgb * u_EnableTextures.w;
  #endif
