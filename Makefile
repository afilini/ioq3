...
ifeq ($(PLATFORM),browser)
  ...
  CLIENT_CFLAGS += -O0 -s ASSERTIONS=2 -g\
    -s MAIN_MODULE=1 \
    -s USE_SDL=2  -s LEGACY_GL_EMULATION=1     -s RESERVED_FUNCTION_POINTERS=1 \
    -s TOTAL_MEMORY=300MB \
    -s NO_EXIT_RUNTIME=1 \
    -s GL_UNSAFE_OPTS=0 \
    $(OPTIMIZE)  --shell-file shell.html

  SERVER_CFLAGS += -O0 -s ASSERTIONS=2 -g\
    -s MAIN_MODULE=1     -s RESERVED_FUNCTION_POINTERS=1 \
